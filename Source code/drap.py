...
class DARP:
    ...
    def show_map(self):
        ...
        for i in range(len(img[0])):
          for j in range(len(img[0])):
            current_node = img[i][j]
            if(current_node == 0):
              img[i][j] = 2
            elif(current_node == 2):
              img[i][j] = 0
        plt.figure()
        # plt.imshow(img, vmin=0, vmax=1, cmap = "binary")
        plt.imshow(img,vmin=0, vmax=2, cmap = "inferno")
        # plt.imshow(img, cmap=plt.cm.hot, interpolation='nearest', vmin=0, vmax=2)
        plt.show()
    ...
    def divideRegions(self):
        ...
        while self.termThr <= self.dcells and not success and not cancelled:
            ...
            # while iteration < 1 and not cancelled:
